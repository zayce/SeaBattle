...
# points1 - cgbcjr relf vs rkbryekb vsirjq
...
def on_closing():
    global app_running
    if messagebox.askokcancel("Выйти из игры", "Хотите выйти из игры?"):
        app_running = False
        tk.destroy()
    ...
    for i in range(0, s_x):
        for j in range(0, s_y):
            if enemy_ships2[j][i] > 0:
                ...
                if points2[j][i] != -1:
                    color = "green"
                _id = canvas.create_rectangle(size_canvas_x + menu_x + i*step_x, j * step_y, size_canvas_x + menu_x + i * step_x +step_x, j*step_y+step_y, fill=color)
...
b0 = Button(tk, text="Показать коабли Игрок №1", command=button_show_enemy1)
b0.place(x=size_canvas_x + 20, y=30)

b1 = Button(tk, text="Показать коабли Игрок №2", command=button_show_enemy2)
...
def draw_point2(x, y, offset_x=size_canvas_x + menu_x):
    ...
    if enemy_ships2[y][x] > 0:
        ...
        id2 = canvas.create_rectangle(offset_x + x * step_x + step_x // 2 - step_x // 10, y * step_y, offset_x + x * step_x + step_x // 2 + step_x // 10, y * step_y + step_y, fill=color)
        list_ids.append(id1)
        list_ids.append(id2)
...
def generate_enemy_ships():hiy
    global  ships_list
    enemy_ships = []

    sum_1_all_sips = sum(ships_list)
    sum_1_enemy = 0

    # print("sum: ", sum_1_all_sips)

    while sum_1_enemy != sum_1_all_sips:
